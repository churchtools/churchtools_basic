...
/**
 * Gibt zurück, ob Person Leiter, coleiter oder Supervisor ist (nicht MA) => True ansonsten false
 * @param p_id
 * @param g_id
    ...
    if (a.gruppe!=null) {
      $.each(a.gruppe, function (i, b) {
        if ((b.id==g_id) && 
          ...
          // Einträge nun in ein Array packen und später dann sortiert ausgeben.
          var entry=new Array();
          entry.id=a.id;
...
GroupView.prototype.renderEditEntry = function (id, fieldname) {
  ...
  var elem = this.showDialog("Ver&auml;nderung der Gruppe", "", 500, 600, {
    "Speichern": function() {
      ...
      // Lösche die Geoinfos, falls es da ein Update bei der Adresse gab.
