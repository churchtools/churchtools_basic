      ...
      global $files_dir;
        ...
        if ($this->file->save($uploadDirectory . $filename . '.' . $ext)){
          ...
          if (rename($filename_old, $files_dir."/fotos/".$filename)) {
              // Get new dimensions
              $ret = getimagesize($files_dir."/fotos/".$filename);
              if (!$ret) return array('error'=>'Fehler bei der Ermittlung des Bildformates. Bitte valides JPG-Bild hochladen.');

              list($width, $height, $imagetype) = $ret;

              if ($imagetype!=IMAGETYPE_JPEG && $imagetype!=IMAGETYPE_JPEG2000 && $imagetype!=IMAGETYPE_PNG)
                return array('error'=>'Bildformat wird nicht unterstüzzt');
              ...
              if (!$image_p) return array('error'=>'Error in imagecreatetruecolor');

              if ($imagetype==IMAGETYPE_PNG) {
                $image = imagecreatefrompng($files_dir."/fotos/".$filename);
              }
              else {
                $image = imagecreatefromjpeg($files_dir."/fotos/".$filename);
              }
              if (!$image) return array('error'=>'Error in imagecreatefrom_XXX');

              $ret = imagecopyresampled($image_p, $image, 0, 0, 0, 0, $new_width, $new_height, $width, $height);
              if (!$ret) return array('error'=>'Error in imagecopyresampled');

              // Output
              $ret = imagejpeg($image_p, $files_dir."/fotos/".$filename, 100);
              if (!$ret) return array('error'=>'Error in imagejpeg');
