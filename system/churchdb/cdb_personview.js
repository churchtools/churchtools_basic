  ...
  searchEntry.catcomplete({
    ...
    select: function(a,item) {
      if (item.item.value.indexOf("CREATE:")==0) {
        ...
        // Leider muß ich hier TimeOut setzen, denn sonst ist der Wert noch nicht in der Oberfläche angekommen...
        window.setTimeout(function(){        
...
PersonView.prototype.addSecondMenu = function() {
  rows = new Array();
  // Gruppenfunktionen sind nur erlaubt, wenn ich Leiter der ausgewählten Gruppe bin, oder ich Schreibrechte habe
  rows.push("<p></p>");
...
/**
 * value: Welche Aktion wurde gewählt
 * param: ein belibiegr Parameter zur Übergabe
 */
...
PersonView.prototype.getListHeader = function() {  
  ...
  // Diese Zeilen dienen beim ersten Aufruf über Direkteinstieg mit Id dafür, dass der Datenssatz
...
function _checkGroupFilter(a, filter, z) {
  ...
  if ((filter["filterFilter "+z]==null) || (filter["filterFilter "+z]==0)) {
    ...
    if ((filter["filterTeilnehmerstatus "+z]!=null)) {
      if (a.gruppe==null) return false;
        ...
        dabei=false;
        // Hier ist kein Zusammenhang zwischen den anderen, d.h. hier muß ich auch noch suchen, 
  ...
  // Filter "War in" der Gruppe
    ...
    if (a.oldGroups!=null) {
        ...
        if (og.id==a.id) {
          if (((filter["filterGruppe "+z]!=null) && (og.gp_id==filter["filterGruppe "+z]))
            ...
            if ((filter["filterGruppeWarInVon "+z]==null) && (filter["filterGruppeWarInBis "+z]==null)) {
              // Ohne Datum muß ich nur schauen, dass es einen Status -99 gibt, also dass er raus ist.
              // bzw. ob der Teilnehmerstatus erfüllt wurde, 
              if (((filter["filterTeilnehmerstatus "+z]==null) && (og.leiter==-99))
                || (og.leiter==filter["filterTeilnehmerstatus "+z])) dabei=true;
            } 
            else {
              // Wenn es ein Datum gibt, dann speichere ich mir die Daten für jede Gruppen-ID in ein Array, s.u. 
      ...
      // Wenn es Datumsfilterung gibt, dann müssen wir alle alten Einträge sortieren und interpretieren
      if (filter["filterGruppeWarInBis "+z]!=null) {
        $.each(arr2, function(k,og) {
          ...
          var startdatum="";
          $.each(b, function(i,c) {            
            // Entweder wird nicht nach TNStatus gefiltert, dann ist >0 wichtig, oder es muß eben vergleichen werden.
            if (((filter["filterTeilnehmerstatus "+z]==null) && (c.leiter>=0))
                || (filter["filterTeilnehmerstatus "+z]==c.leiter)) 
              startdatum=c.d;
            // Wenn auch ein -99 da ist bzw. der TN-Status sich geändert hat, dann haben wir einen gültiges Bundle, also los!
...
PersonView.prototype.checkFilter = function(a) {
  ...
  if (searchEntry!="") {
    ...
    $.each(searches, function(k,search) {
      ...
      // Wenn kein Tag, dann nun andere Möglichkeiten testen
...
//                 ((a.spitzname+" "+a.name).toUpperCase().indexOf(search)!=0) &&
  ...
  if (a.searchable) {
    ...
    if ((filter["filterDates"]!=null) && (filter["dateAfter"]!=null) && (filter["dateAfter"].toDateDe()!=null)) {
      // Enth√§lt der Datensatz √ºberhaupt ein Datum?
      if (a[filter["filterDates"]]!=null) {
        flt=a[filter["filterDates"]].toDateEn();
      ...
      $.each(allPersons[id].rels, function (k,b) {
        if (b.beziehungstyp_id==relType.id) {
          if (b.kind_id==id) {
            ...
            // Checken ob er überhaupt die Person sehen darf, kann anderer Bereich sein.
...
/**
 * Ist es überhaupt eine Followup-Gruppe?
 * group=Person.group
 */
 ...
 * @return null oder Anzahl Tage von heute bis zum FollowUp, negativ wenn schon überfällig
 */
...
PersonView.prototype.renderDetails = function (id) {
  ...
  rows.push('<div id="detail" class="detail-view-person">');
    ...
    rows.push('<div class="row-fluid">');
      ...
      // Die kein Recht auf alle Daten haben müssen auch nur wissen, ob Mitglied oder nicht. Nicht die einzelnen Statis.
      else {
      ...
      if (allPersons[id]!=null) {
        $.each(churchcore_sortMasterData(masterData.groupTypes), function(k,a) {
          ...
          if ((masterData.auth.viewalldetails) 
            ...
            // Erst mal alle auf Unused setzen, damit ich hinterher weiß, welche Archiv-Gruppe noch nicht gezeigt wurde
  ...
  $("td[id=detailTD"+id+"] a").click(function() {
    // Lösche den Tooltip, falls es ihn gibt
    t.clearTooltip(true);
    ...
    // Wenn Beziehungen gepflegt werden, gibt es die Standard PersonSelect-Seite
      ...
      // FollowUp Ergänzen, falls die aktuelle Person hier ein FollowUp durchführen soll
...
PersonView.prototype.editPersonAuth = function (id) {
  ...
  this.editDomainAuth(id, allPersons[id].auth, "person", function(id) {
    // Hole mir neue Details für die Person mit der Auth-infos
    churchInterface.jsendRead({func:"getPersonDetails", id:id}, function(ok, json) {
      allPersons[json.id]=cdb_mapJsonDetails(json, allPersons[json.id]);
...
PersonView.prototype.delPersonFromGroup = function (id, g_id, withoutConfirmation) {
  ...
  // Nicht erlaubt, d.h. es handelt sich um einen KG-Leiter, der nur auf "Zu Löschen" setzen darf
  else {
  ...
  if (masterData.fields[fieldname]!=null) {
    ...
    if (masterData.groups==null) {
      ...
      $.each(churchcore_sortData(masterData.groups, "bezeichnung"), function(k,a) {
        // Es sollen nur Gruppen auswählbar sein, deren Abschlußdatum zu alt ist
        if ((a.gruppentyp_id==gt_id) 
            && (a.valid_yn==1) 
            && ((a.abschlussdatum==null) || (a.abschlussdatum.toDateEn()>diff_date))) {
            // Wenn ich die Gruppe sehen darf, darf ich sie auch zuordnen. Und natürlich auf editgroups
...
/**
 * Bereitet die Daten für ein renderSelect vor
...
PersonView.prototype.msg_filterChanged = function (id, oldVal) {
  ...
  // Wenn nicht "Meine Gruppen" geändert wurden, ich aber Meine Gruppen ein intelligente Gruppe gefiltert habe
  ...
  // Wenn "Meine Gruppen" geändert wurde
    ...
    // Wenn es mit "filter" anfängt, dann handelt es sich jetzt um intelligente Gruppen
    ...
    // Wenn es vorher eine intelligente Gruppe war, muß ich nun Filter wieder löschen
...
PersonView.prototype.renderGroupContent = function(g_id) {
  ...
  if (json!=null) {
    ...
    // Kein zu pflegende Gruppe gefunden, also rows wieder lˆschen.
  ...
  $("#cdb_group input, #cdb_group a").click(function(c) {
    if ($(this).val()=="Auswahl absenden") {
      ...
      churchInterface.jsendWrite({ func: "GroupMeeting", sub:"canceled", gt_id: gruppentreffen_id }, function(oi, json) {
        // info lˆschen, damit er neue Infos holt.
...
PersonView.prototype.mailer = function() {
  ...
  $.each(allPersons, function(k, a) {
    if ((counter<=maxMails) && (t.checkFilter(a))) {
      if ((a.email!=null) && (a.email!="")) {
        counter++;
        // Wenn ich das in ein Extra Fenster ausgeben, können auch die Namen dazu
...
//    if (noEmail) alert("Hinweis: Einige Eintr√§ge haben keine E-Mailadresse, diese wurden nicht ber√ºcksichtigt!");
