...
function churchresource_updateBooking($params, $changes = null) {
  ...
  // Only bigChange, if repeat_id is set. Otherwise it is only a time shift.
  // TODO: is it right to treated repeat_id set to 0 as bigChange?
  $bigChange = isset($params["repeat_id"]);
  
  $booking = $oldBooking  = getBooking($params["id"]);
  ...
  // TODO: add resource type title for use in emails, sorting not needed?
  $resources = churchcore_getTableData("cr_resource", "resourcetype_id,sortkey,bezeichnung");
  ...
  if (is_null($changes) && $bigChange) {
    ...
    // TODO: maybe put add/exceptions in one table with a flag set to 1 for add - could it simplify exception handling?
    // get alle exceptions
